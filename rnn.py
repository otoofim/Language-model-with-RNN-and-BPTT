...
class RNN(object):
	...
	def predict(self, x):
		...
		for t in range(len(x)):
			...
			##########################
                  ...
                  one_hot[x[t]] = 1
                  net_in=np.dot(self.V,one_hot)+ np.dot(self.U,s[t-1])
                  s[t] = 1 / (1 + np.exp(np.dot(net_in,-1)))
                  net_out = np.dot(self.W, s[t])
                  y[t] = np.exp(net_out) / np.sum(np.exp(net_out), axis=0)
		#print(y)
		return y,s
	...
	def acc_deltas(self, x, d, y, s):
		...
		for t in reversed(range(len(x))):
			...
			##########################
                   one_hot= np.zeros(self.vocab_size)
                   one_hot[d[t]] = 1
                   sigma_out=np.multiply((one_hot-y[t]),np.ones(len(y[t])))
                   self.deltaW += np.outer(sigma_out,s[t])


                   one_hot2= np.zeros(self.vocab_size)
                   one_hot2[x[t]] = 1
                   f_prime = np.multiply(s[t], (np.ones(len(s[t]))-s[t]))
                   sigma_in = np.multiply(np.dot(self.W.T, sigma_out), f_prime)
                   self.deltaV += np.outer(sigma_in,one_hot2)


                   self.deltaU += np.outer(sigma_in, s[t-1])
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		for t in reversed(range(len(x))):
			...
			##########################
                  ...
                  for eta in range(1,min(steps+1,len(x)+1)):
                     f_prime_t = np.multiply(s[t-eta], (np.ones(len(s[t-eta]))-s[t-eta]))

                     one_hot2= np.zeros(self.vocab_size)
                     one_hot2[x[t-eta]] = 1
                     temp = np.multiply(np.dot(self.U.T, sigma_in), f_prime_t)
                     sigma_in = temp
                     self.deltaV += np.outer(sigma_in,one_hot2)

                     self.deltaU += np.outer(sigma_in, s[t-eta-1])
	...
	def compute_loss(self, x, d):
		...
		##########################
                ...
                correct_word_predictions = o[np.arange(len(d)), d]
	...
	def generate_sequence(self, start, end, maxLength):
		...
		##########################
		counter=0
		while (not sequence[-1] == end) and counter<maxLength:
			next_word_probs,_ = self.predict(sequence)
			sampled_word=multinomial_sample(next_word_probs[-1])
			while sampled_word == word_to_num['UUUNKKK']:
				sampled_word=multinomial_sample(next_word_probs[-1])
			loss += (-1*np.log(next_word_probs[-1][sampled_word]))
			sequence.append(sampled_word)
			counter += 1
		loss = (loss)
		return sequence, loss
	...
	def train(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100, min_change=0.0001, log=True):
		...
		print("best observed loss was {0}, at epoch {1}".format(best_loss, (best_epoch+1)))
                #with open("result.txt", "a") as myfile:
                   #myfile.write("best observed loss was {0}, at epoch {1}  |||  ".format(best_loss, (best_epoch+1)))
...
if __name__ == "__main__":
	...
	if mode == "estimate":
		...
		best_lr = -1
                #r = RNN(vocab_size,25)
                #temp=r.train(X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, back_steps=5)

                for num_hidd_un in [25,35, 50, 65]:
                   for look_back in [0,2,5, 6]:
                      for learning_rate in [0.5, 0.1,0.05,0.001]:
                         r = RNN(vocab_size,num_hidd_un)
                         temp=r.train(X, D, X_dev, D_dev, epochs=10, learning_rate=learning_rate, back_steps=look_back)
                         #with open("result.txt", "a") as myfile:
                            #myfile.write("hdim: {0}, lookback: {1}, lr: {2}\n".format(num_hidd_un,look_back,learning_rate))
                         if temp<best_loss:
                            best_lost=temp
                            best_hdim=num_hidd_un
		            best_lookback=look_back
		...
		print("\nestimation finished.\n\tbest loss {0}:\tbest hidden/lookback/learn rate: {1}/{2}/{3}".format(best_loss, best_params[0], best_params[1], best_params[2]))
                #with open("result.txt", "a") as myfile:
                   #myfile.write("estimation finished.\n\tbest loss {0}:\tbest hidden/lookback/learn rate: {1}/{2}/{3}".format(best_loss, best_params[0], best_params[1], best_params[2]))
	...
	if mode == "train":
		...
		adjusted_loss = -1
                ...
                #run_loss = r.train(X_train, D_train, X_dev, D_dev, epochs=10, learning_rate=lr, back_steps=lookback)
                #np.save("rnn.U.npy", r.U)
                #np.save("rnn.V.npy", r.V)
                #np.save("rnn.W.npy", r.W)
                r.U,r.V,r.W = np.load("rnn.U.npy"), np.load("rnn.V.npy"), np.load("rnn.W.npy")
                run_loss = r.compute_mean_loss(X_dev, D_dev)
                X_dev, D_dev = seqs_to_lmXY(S_dev)
                mean_loss = r.compute_mean_loss(X_dev, D_dev)
                unadjusted_perp = np.exp(mean_loss)
                adjusted_loss=adjust_loss(mean_loss,fraction_lost,q)
                adjusted_perp = np.exp(adjusted_loss)
		print("Mean lost: {0}".format(mean_loss))
                print("run lost: {0}".format(run_loss))
		print "Unadjusted: %.03f" % (unadjusted_perp)
		print ("Adjusted for missing vocab: {0}".format((adjusted_perp)))
                #with open("train.txt", "a") as myfile:
                   #myfile.write("Mean lost: {0}\n".format(np.exp(mean_loss)))
                   #myfile.write("Unadjusted: %.03f\n" % np.exp(unadjusted_perp))
                   #myfile.write("Adjusted for missing vocab: %.03f\n" % np.exp(adjusted_perp))
	...
	if mode == "generate":
		...
		# --- your code here --- #
		##########################
		seq,perp=r.generate_sequence(word_to_num['<s>'], word_to_num['</s>'], maxLength)
		sentence_str = [num_to_word[x] for x in seq]
		print(sentence_str,perp)
		# predict something
